...
def getModel(net_type, inputs, outputs):
    ...
    if (net_type == '3conv3fc_1d'):
        return BBB3Conv3FC_1D(2 * outputs,inputs,init_log_noise=0)
    elif(net_type == '3liner'):
        return BBB3Liner(2 * outputs, inputs,init_log_noise=0)
    else:
...
def test_uncertainty(net, testset, data='ccpp'):
    ...
    # aleatoric = aleatoric.reshape([aleatoric.shape[0]])
